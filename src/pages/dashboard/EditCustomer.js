...
function EditCustomer({ uncompleted }) {
  ...
  const [qrsrc, setqrsrc] = useState(
    `https://qr.speedlab-resaults.com/#/customer/result?customer_id=${customer_id}`
  );
  ...
  useEffect(async () => {
    try {
      ...
      setpassport_expiry(customer.data[0].passport_expiry);
      setIssueCode(customer.data[0].IssueCode);
      setApprovedCode(customer.data[0].ApprovedCode);
      setUserImage(customer.data[0].customer_image);
